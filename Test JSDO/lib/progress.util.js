...
(function () {
    ...
    /*
     * Converts the specified filter object to an OpenEdge Where String.
     *
     ...
     */
    progress.util._convertToWhereString = function (tableRef, filter) {
        var result = [],
        ...
        for (idx = 0, length = filters.length; idx < length; idx=idx+1) {
            ...
            if (filter.filters) {
                filter = progress.util._convertToWhereString(tableRef, filter);
            } else {
                ...
                if (operator && value !== undefined) {
                    ...
                    if (operator === "INDEX" || operator === "R-INDEX") {
                        if (type !== "string") {
                            throw new Error("The operator " + filter.operator + " requires a string value");
                        }
